...
|-----------|-------------|--------|
| [`swop/cli-wrapper/codex-auth-isolation-discovery`](work-items/codex-auth-isolation-discovery/spec.md) | Validate how to isolate `codex` auth/config per account/process in WSL/mac | planned |
| [`swop/cli-wrapper/account-sandbox-manager`](work-items/account-sandbox-manager/spec.md) | Manage per-account sandboxes (create/list/remove), permissions and layout | planned |
